...
func sumPoolVerify(input: Tensor, kernelSize: [Int], stride: [Int], padMode: PaddingMode,order: TensorChannelOrder) -> Tensor {
...
public class OperatorDelegateConvSumPool2DOp: OperatorDelegateConvPool2DOp {
	override public func compare() {
		...
		for (input, output) in zip(inputTensors, resultTensors) {
			let verifyTensor = sumPoolVerify(input: input, kernelSize: kernelSize, stride: stride, padMode: padMode, order:order)
...
class Sumpool2D_op_test: XCTestCase {
	...
	func test() {
		let testCase = Pool2DOperatorTest<SumPool2DOperator, OperatorDelegateConvSumPool2DOp>()
		testCase.testAll()
	}
