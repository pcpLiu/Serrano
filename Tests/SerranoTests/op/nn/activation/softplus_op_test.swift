...
class SoftplusOpDelegate: OperatorDelegateConvUnaryOp {
    ...
    required public convenience init(compareBlock: ((Tensor, Tensor) -> Void)?) {
        let blcok =  {(rawTensor: Tensor, resultTensor: Tensor) -> Void in
            ...
            for i in 0..<rawTensor.count {
                let val = log(exp(readerReader[i]) + 1.0)
        ...
        self.gradVerifyBlock = {(grads: [String : DataSymbolSupportedDataType], inputs:[Tensor]) -> Void in
            for (index, input) in inputs.enumerated() {
                ...
                for i in 0..<input.count {
                    let val:Float = exp(input.floatValueReader[i]) / (1.0 + exp(input.floatValueReader[i]))
...
class SoftplusOpTest: XCTestCase {
    ...
    func test() {
        let testCase = UnarOpTest<SoftplusOpDelegate, SoftplusOperator>()
//        testCase.testAll()
