        ...
        self.gradVerifyBlock = {(grads: [String : DataSymbolSupportedDataType], inputs:[Tensor]) -> Void in
            for (index, input) in inputs.enumerated() {
                ...
                for i in 0..<input.count {
                    let val = 1.0 / sqrt(1 + input.floatValueReader[i] * input.floatValueReader[i])
...
class ArcsinhOpTest: XCTestCase {
    ...
    func test() {
        let testCase = UnarOpTest<ArcsinhOpDelegate, ArcsinhOperator>()
        testCase.testAll()
    }
}
