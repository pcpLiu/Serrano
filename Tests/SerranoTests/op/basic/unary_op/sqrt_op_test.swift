        ...
        self.gradVerifyBlock = {(grads: [String : DataSymbolSupportedDataType], inputs:[Tensor]) -> Void in
            for (index, input) in inputs.enumerated() {
                ...
                for i in 0..<input.count {
                    let val:Float = 1 / (2 * sqrt(input.floatValueReader[i]))
...
class SqrtOpTest: XCTestCase {
    ...
    func test() {
        let testCase = UnarOpTest<SqrtOpDelegate, SqrtOperator>()
        testCase.testAll()
    }
