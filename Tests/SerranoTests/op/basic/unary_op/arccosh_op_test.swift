        ...
        self.gradVerifyBlock = {(grads: [String : DataSymbolSupportedDataType], inputs:[Tensor]) -> Void in
            for (index, input) in inputs.enumerated() {
                ...
                for i in 0..<input.count {
                    let val = 1.0 / (sqrt(input.floatValueReader[i] - 1) * sqrt(input.floatValueReader[i] + 1))
...
class ArccoshOpTest: XCTestCase {
    ...
    func test() {
        let testCase = UnarOpTest<ArccoshOpDelegate, ArccoshOperator>()
        testCase.testAll()
    }
}
