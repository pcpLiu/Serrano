    ...
    required public convenience init(compareBlock: ((Tensor, Tensor) -> Void)?) {
        let blcok =  {(rawTensor: Tensor, resultTensor: Tensor) -> Void in
            ...
            for i in 0..<rawTensor.count {
                let val = floor(readerReader[i])
                XCTAssertEqual(val, resultReader[i], accuracy: max(abs(val*0.001), 0.001))
            }
        }
...
class FloorOpTest: XCTestCase {
    ...
    func test() {
        let testCase = UnarOpTest<FloorOpDelegate, FloorOperator>()
        testCase.testAll()
    }
}
