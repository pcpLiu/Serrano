        ...
        self.gradVerifyBlock = {(grads: [String : DataSymbolSupportedDataType], inputs:[Tensor]) -> Void in
            for (index, input) in inputs.enumerated() {
                ...
                for i in 0..<input.count {
                    let val = 1.0 / sqrt(1.0 - input.floatValueReader[i] * input.floatValueReader[i])
...
class ArcsinOpTest: XCTestCase {
    ...
    func test() {
        let testCase = UnarOpTest<ArcsinOpDelegate, ArcsinOperator>()
        testCase.testAll()
    }
}
