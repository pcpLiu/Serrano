...
class CosOpDelegate: OperatorDelegateConvUnaryOp {
        ...
        self.gradVerifyBlock = {(grads: [String : DataSymbolSupportedDataType], inputs:[Tensor]) -> Void in
            for (index, input) in inputs.enumerated() {
                ...
                for i in 0..<input.count {
                    let val = -sin(input.floatValueReader[i])
...
class CosOpTest: XCTestCase {
    ...
    func test() {
        let testCase = UnarOpTest<CosOpDelegate, CosOperator>()
        testCase.testAll()
    }
}
