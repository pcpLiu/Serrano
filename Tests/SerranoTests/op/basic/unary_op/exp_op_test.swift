    ...
    required public convenience init(compareBlock: ((Tensor, Tensor) -> Void)?) {
        let blcok =  {(rawTensor: Tensor, resultTensor: Tensor) -> Void in
            ...
            for i in 0..<rawTensor.count {
                let val = exp(readerReader[i])
                if val.isNaN || val.isInfinite || resultReader[i].isNaN || resultReader[i].isInfinite { continue }
                XCTAssertEqualWithAccuracy(val, resultReader[i], accuracy: val * 0.0001)
            }
        }
...
class ExpOpTest: XCTestCase {
    ...
    func test() {
        let testCase = UnarOpTest<ExpOpDelegate, ExpOperator>()
        testCase.testAll()
    }
