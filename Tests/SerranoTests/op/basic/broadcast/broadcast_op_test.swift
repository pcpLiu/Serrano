...
//    }
    ...
    */
     ...
     */
    func testCanBroadcast() {
        let numCase = 100
        for i in 0..<numCase {
            print("Test \(i+1)...")
            
            // target shape
            let targetShape = randomShape(dimensions: randomInt([1, 6]), dimensionSizeRange: [1, 10], dataType: .int)
            
            // shapes
            let shapes: [TensorShape]
            if i % 2 == 0 {
                // valid
                shapes = generateShapes(targetShape: targetShape, valid: true)
            } else {
                shapes = generateShapes(targetShape: targetShape, valid: false)
            }
            let tensor = randomTensor(fromShape: shapes.first!)
            
            let check = BroadcastOperator.canBroadcast(tensor, targetShape: targetShape)
            if i % 2 == 0 {
                XCTAssertTrue(check)
            } else {
     ...
     */
    func testAxisBrodcsting() {
        let numCase = 100
        for i in 0..<numCase {
            print("Finish Test \(i+1)...")
            
            let targetShape = randomShape(dimensions: randomInt([1, 6]), dimensionSizeRange: [1, 10], dataType: .int)
            print("target shape: ", targetShape.shapeArray)
            
            let shapes = generateShapes(targetShape: targetShape, valid: true)
            let tensor = randomTensor(fromShape: shapes.first!)
            print("tensor shape: ", tensor.shape.shapeArray)
            
            // checking axis
            var checkAxis = [Int]()
            let rTargetShapeArray = targetShape.reversedShapArray()
            let rShapeArray = tensor.shape.reversedShapArray()
            for (i, dim) in rShapeArray.enumerated() {
                if dim == 1 && rTargetShapeArray[i] != 1 {
                    checkAxis.append(rShapeArray.count - i - 1)
                }
            }
            
            let axis = BroadcastOperator.axisBroadcasting(tensor, targetShape: targetShape)
            XCTAssertEqual(axis, checkAxis)
            print("checkAxis ", checkAxis)
            print("axis ", axis)
    ...
    public func testGradCompute() {
        ...
        for i in 0..<numCase {
            ...
            let op = BroadcastOperator(targetShape: targetShape)
            op.inputTensors = [tensor]
            op.outputTensors = [Tensor(repeatingValue:0.0, tensorShape: targetShape)]
            
            op.compute()
            let grads = op.gradCompute(.CPU)
            for (_, grad) in grads {
                let gradTensor = grad.tensorValue
                for valueIndex in 0..<gradTensor.count {
                    XCTAssertEqual(1.0, gradTensor.floatValueReader[valueIndex])
                }
            }
